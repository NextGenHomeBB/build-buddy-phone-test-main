-- Improve project creation logic to only assign manager role when JWT is present
...
BEGIN
  ...
  creator := nullif(current_setting('request.jwt.claims', true)::json->>'sub','')::uuid;
  ...
  IF sched_id IS NULL THEN
    INSERT INTO schedules (work_date, created_by) 
    VALUES (work_date, COALESCE(creator, system_user_id)) 
  ...
  LOOP
    ...
    IF proj_id IS NULL THEN
      ...
      -- Link importing user as manager (only if JWT present)
      IF creator IS NOT NULL THEN
        INSERT INTO user_project_role (user_id, project_id, role)
        VALUES (creator, proj_id, 'manager')
