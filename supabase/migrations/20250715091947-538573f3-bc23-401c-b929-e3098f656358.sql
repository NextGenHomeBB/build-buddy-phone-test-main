-- Fix the import_schedule_tx function to prevent duplicate schedule items
...
BEGIN
  ...
  LOOP
    ...
    SELECT id INTO sched_item_id
    FROM schedule_items
    WHERE schedule_id = sched_id
      AND lower(trim(address)) = lower(trim(it->>'address'))
      AND start_time = (it->>'startTime')::time
      AND end_time = (it->>'endTime')::time;
