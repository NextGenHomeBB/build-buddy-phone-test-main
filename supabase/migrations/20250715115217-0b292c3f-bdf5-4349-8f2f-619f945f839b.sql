-- 1.1  new column (safe if already exists)
ALTER TABLE projects
  ADD COLUMN IF NOT EXISTS created_by uuid;

-- 1.2  RLS read policy for project creators and assigned users
...
-- 1.3  Back-fill existing projects - make current user the creator where null
DO $$
DECLARE
  _uid uuid;
BEGIN
  -- Try to get the current user from JWT
  BEGIN
    _uid := (current_setting('request.jwt.claims', true)::json->>'sub')::uuid;
  EXCEPTION WHEN OTHERS THEN
    -- If no JWT context, use the first admin user
    SELECT user_id INTO _uid FROM profiles WHERE role = 'admin' LIMIT 1;
  END;

  IF _uid IS NOT NULL THEN
    -- Add manager role for existing projects without created_by
    INSERT INTO user_project_role (user_id, project_id, role)
    SELECT _uid, id, 'manager'
    FROM   projects
    ...
    UPDATE projects
    SET    created_by = COALESCE(created_by, _uid)
    WHERE  created_by IS NULL;
  END IF;
END $$;

-- 2. Update import_schedule_tx function to stamp creator UID
...
DECLARE
  ...
  creator_uid      uuid;
BEGIN
  -- Get the authenticated user from JWT context (the manager triggering import)
  BEGIN
    creator_uid := (current_setting('request.jwt.claims', true)::json->>'sub')::uuid;
  EXCEPTION WHEN OTHERS THEN
    creator_uid := NULL;
  END;
